...
def pdf_ocr_read_hun():
    ...
    # 读取已处理文件列表
    fail_file_ckpt = '../../../Ingredient_Search/data_3/fail_files_ocr_hun.json'
    ...
    main_folder_path = '../../../Ingredient_Search/data_hun'
    file_pairs = []
    # 遍历主文件夹
    for subdir, dirs, files in os.walk(main_folder_path):
        for file in files:
            # 检查文件名是否为目标文件
            if '备案凭证.pdf' in files and '产品说明书.pdf' in files:
                # 构建两个文件的完整路径
                record_certificate_path = os.path.join(subdir, '备案凭证.pdf')
                product_manual_path = os.path.join(subdir, '产品说明书.pdf')
                file_pairs.append((record_certificate_path, product_manual_path))

    # 处理对应省份的数据
    for file_pair in tqdm(file_pairs):
        pdf_path = file_pair[0]
        if pdf_path in finished_pdf:
            print(f"文件 {pdf_path} 已处理，跳过")
            continue
        # 将PDF文件转换为图像列表
        try:
            images = convert_from_path(pdf_path, poppler_path=r'../../../Ingredient_Search/poppler/poppler-24.08.0/Library/bin')
        except Exception as e:
            print(f"处理文件 {pdf_path} 时发生错误: 可能不是个PDF文件")
            continue
        pdf_path = file_pair[1]
        if pdf_path in finished_pdf:
            print(f"文件 {pdf_path} 已处理，跳过")
            continue
        # 将PDF文件转换为图像列表
        try:
            images.extend(convert_from_path(pdf_path, poppler_path=r'../../../Ingredient_Search/poppler/poppler-24.08.0/Library/bin'))
        except Exception as e:
            print(f"处理文件 {pdf_path} 时发生错误: 可能不是个PDF文件")
            continue
